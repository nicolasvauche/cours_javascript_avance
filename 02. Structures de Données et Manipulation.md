...
- **some() :** Vérifie si au moins un élément du tableau passe le test implémenté par la fonction fournie. Renvoie true
  ou false.
    ```javascript
    let numbers = [1, 3, 5, 8];
...
- **indexOf() :** Renvoie le premier indice auquel un élément donné peut être trouvé dans le tableau, ou -1 si l'élément
  n'est pas présent. Recherche de gauche à droite. Renvoie un entier.
    ```javascript
    let fruits = ["pomme", "orange", "banane"];
...
- **flat() :** Aplatit les tableaux imbriqués à une profondeur spécifiée. Renvoie un nouveau tableau.
    ...
    ```
- **keys() :** Renvoie un nouvel objet Array Iterator qui contient les clés pour chaque index du tableau. Renvoie un
  objet Array Iterator.
    ```javascript
    let fruits = ["pomme", "orange", "banane"];
...
- **values() :** Renvoie un nouvel objet Array Iterator qui contient les valeurs pour chaque index du tableau. Renvoie
  un objet Array Iterator.
    ```javascript
    let fruits = ["pomme", "orange", "banane"];
...
- **entries() :** Renvoie un nouvel objet Array Iterator qui contient les paires clé/valeur pour chaque index du
  tableau. Renvoie un objet Array Iterator.
    ```javascript
    let fruits = ["pomme", "orange", "banane"];
...
Map respecte l'ordre d'insertion des éléments. Set, d'autre part, est une collection de valeurs uniques. Set peut être
utilisé pour supprimer les doublons d'un tableau ou pour effectuer des opérations mathématiques comme l'union,
l'intersection et la différence. Les deux structures de données sont itérables et disposent d'une variété de méthodes
utiles.
